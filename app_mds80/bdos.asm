...
bdose:	;arrive here from user programs
	...
	xra a
	sta fcbdsk
	sta resel ;fcbdsk,resel=cpmfalse
...
diskf	equ	($-functab)/2	;disk funcs
	...
	dw	set_ro,func29,func30,func31
	dw	func32,func33,func34,func35
	dw	func36,func37,func_ret,func_ret	;
	dw	func40					;
...
selsub:	;report select error
	lxi h,selmsg
	jmp wait_err ;wait console before boot
;
rodsub:	;report write to read/only disk
	lxi h,rodmsg
	jmp wait_err ;wait console
;
...
conb0:
	...
	sta kbchar
conb1:
	;return with cpmtrue set in accumulator
	mvi a,1
	ret
;
conout:
	;compute character position/write console char from C
	;compcol = cpmtrue if computing column position
	lda compcol
	ora a
	jnz compout
...
;	
nowrite:
	;return cpmtrue if dir checksum difference occurred
	lhld rodsk
	lda curdsk
	mov c,a
	call hlrotr
...
;
newchecksum:
	mvi c,cpmtrue ;drop through to compute new checksum
checksum:
	;compute current checksum record and update the
	;directory element if C=cpmtrue, or check for = if not
	;drec < chksiz?
	lhld drec
	xchg
	lhld chksiz
	...
	dad d ;HL = .check(drec)
	pop b ;recall cpmtrue=0ffh or cpmfalse=00 to C
...
;
read_dir:
	;read next directory entry, with C=cpmtrue if initializing
	lhld dirmax
...
initialize:
	...
	;lret = cpmfalse ;set to cpmtrue if $ file exists
...
searchn:
	...
	call end_of_dir
	jz search_fin ;skip to end if so
...
open_reel:
	...
	;if possible.  RMF is cpmtrue if in read mode
	xra a
	sta fcb_copied ;set cpmtrue if actually copied
	...
	lxi h,fcb_copied ;cpmtrue if the fcb was copied to directory
...
open_mod:
	...
	jz open_rerr ;cannot overflow to zero
...
open_reel0:
	...
	jz open_rerr ;sets lret = 1
	;try to extend the current file
	call make
	;cannot be end of directory
	call end_of_dir
	jz open_rerr ;with lret = 1
	jmp open_reel2
open_reel1:
...
diskread:	;(may enter from seqdiskread)
	mvi a,cpmtrue
	sta rmf ;read mode flag = cpmtrue (open$reel)
...
select:
	...
	cz sel_error ;returns cpmtrue if select ok
...
func12:
	...
	mvi a,dvers
	jmp sta_ret ;lret = dvers (high = 00)
;	ret ;jmp goback
;
func13:
...
func17:
	...
	mvi c,0 ;length assuming '?' cpmtrue
	xchg		;was lhld info		
	mov a,m
	cpi '?' ;no reselect if ?
...
;	data areas
;
compcol:db	0	;cpmtrue if computing column position
...
fcb_copied:
	ds	byte	;set cpmtrue if copy$fcb called
...
single:	ds	byte	;set cpmtrue if single byte allocation map
