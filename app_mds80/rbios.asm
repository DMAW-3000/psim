...
;
	org	patch
cpmb	equ	6100h	;base of cpm console processor
...
signon:	;signon message: xxk cp/m vers y.y
	db	cr,lf,lf
	db	'32'	;memory size filled by relocator
	db	'k CP/M vers 2.2'
	db	cr,lf,0
...
;	start.
;
	lxi	sp,buff+80h	;using dma - thus 80 thru ff available for stack
;
...
;	otherwise too many retries
	lxi	h,bootmsg
	call	prmsg
	jmp	mon80	;mds hardware monitor
;
bootmsg:
	db	'?boot',0
;
...
read:	;read next disk record (assuming disk/trk/sec/dma set)
	...
	call	setfunc
	jmp	waitio	;perform read function
...
;
	...
	mvi	b,iopb >> 8	;high address for iopb
...
wready:	;not ready, treat as error for now
	call	inbyte		;clear resGult byte
	jmp	trycount
;
...
datsiz	equ	$-begdat;size of data are