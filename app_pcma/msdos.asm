...
; INTBASE:      ABORT							(INT 20H)
; INTBASE+4:    COMMAND							(INT 21H)
; INTBASE+8:    BASE EXIT ADDRESS				(INT 22H)
; INTBASE+C:    CONTROL-C ABORT					(INT 23H)
; INTBASE+10H:  FATAL ERROR ABORT				(INT 24H)
; INTBASE+14H:  BIOS DISK READ					(INT 25H)
; INTBASE+18H:  BIOS DISK WRITE					(INT 26H)
; INTBASE+40H:  Long jump to CALL entry point	(INT 27H)
...
EXTENT  EQU     12
RECSIZ  EQU     14       ;DW Size of record (user settable)
FILSIZ  EQU     16       ;DW (DD) Size of file in bytes (includes next DW)
DRVBP   EQU     18       ;DW BP for SEARCH FIRST and SEARCH NEXT
FDATE   EQU     20       ;DW Date of last writing
FTIME   EQU     22       ;DW Time of last writing
DEVID   EQU     24       ;DB Device ID number, bits 0-5
                        ;bit 7=0 for file, bit 7=1 for I/O device
                        ;If file, bit 6=0 if dirty
                        ;If I/O device, bit 6=0 if EOF (input)
FIRCLUS EQU     25       ;DW First cluster of file
LSTCLUS EQU     27       ;DW Last cluster accessed
CLUSPOS EQU     29       ;DW Position of last cluster accessed
NR      EQU     32       ;DB Next record
RR      EQU     33               ;Random record
FILDIRENT       = FILSIZ                ;Used only by SEARCH FIRST and SEARCH NEXT

BIOSSEG EQU     70H

; Field definition for Drive Parameter Block, offsets

DEVNUM  EQU     0       ;DB I/O driver number
DRVNUM  EQU     1       ;DB Physical Unit number
SECSIZ  EQU     2       ;DW Size of physical sector in bytes
CLUSMSK EQU     4       ;DB Sectors/cluster - 1
CLUSSHFT EQU    5       ;DB Log2 of sectors/cluster
FIRFAT  EQU     6       ;DW Starting record of FATs
FATCNT  EQU     8       ;DB Number of FATs for this drive
MAXENT  EQU     9       ;DW Number of directory entries
FIRREC  EQU     11      ;DW First sector of first cluster
MAXCLUS EQU     13      ;DW Number of clusters on drive + 1
FATSIZ  EQU     15      ;DB Number of records occupied by FAT
FIRDIR  EQU     16      ;DW Starting record of directory
FAT     EQU     18      ;DW Pointer to start of FAT

DPBSIZ  EQU     20      ;Size of the structure in bytes
DIRSEC  EQU     FIRREC  ;Number of dir. sectors (init temporary)
DSKSIZ  EQU     MAXCLUS ;Size of disk (temp used during init only)
...
OUTCH:
        ...
        CALL    STATCHK
        POP     AX
        CALLF   BIOSSEG:BIOSOUT 
        TEST    BYTE PTR CS:[PFLAG],-1
        JZ      RET18
        CALLF	BIOSSEG:BIOSPRINT 
RET18:  RET

STATCHK:
        CALLF   BIOSSEG:BIOSSTAT 
        JZ      RET18
        CMP     AL,'S'-'@'
        JNZ     NOSTOP
        CALLF   BIOSSEG:BIOSIN          ;Eat Cntrl-S
INCHK:
        CALLF   BIOSSEG:BIOSIN 
        CMP     AL,'P'-'@'
        JZ      PRINTON
        CMP     AL,'N'-'@'
        JZ      PRINTOFF
        CMP     AL,'K'-'@'		; CNTL-C mapped to CNTL-K
        JNZ     RET18
; Ctrl-C handler.
...
PRINTON:
        MOV     BYTE PTR CS:[PFLAG],1
        RET

PRINTOFF:
        MOV     BYTE PTR CS:[PFLAG],0
        RET
...
; Carry set if bad file name or drive
        ...
        MOV     WORD PTR CS:[CREATING],0E500H   ;Not creating, not DEL *.*
        MOV     AX,CS
        MOV     ES,AX
        MOV     DI,NAME1
        MOV     SI,DX
...
; All other registers destroyed
        ...
        CALLF	BIOSSEG:BIOSDSKCHG      ;See what BIOS has to say
        JC      FATERR
        CALL    GETBP
...
BADCLOSEJ:
        ...
        MOV     [SI],CX
        MOV     DX,WORD PTR ES:[DI+FILSIZ]
        MOV     [SI+2],DX
        MOV     DX,WORD PTR ES:[DI+FILSIZ+2]
        MOV     [SI+4],DX
        MOV     DX,ES:[DI+FDATE]
        MOV     [SI-2],DX
        MOV     DX,ES:[DI+FTIME]
...
; BP preserved. All other registers destroyed.
        ...
        PUSH    CX
        PUSH    DX
        CALLF	BIOSSEG:BIOSWRITE 
        POP     DX
        POP     DI
        POP     BX
        POP     BP
        JNC     RET9
        MOV     BYTE PTR CS:[READOP],1
        CALL    HARDERR
        CMP     AL,1            ;Check for retry
        JZ      DWRITE
        RET
...
SAVFIR:
        ...
        MOV     AX,BX
        XOR     DX,DX
        DIV     WORD PTR [BP+SECSIZ]     ;How many whole sectors?
        MOV     [SECCNT],AX
...
ENDRDDEV:
        ...
        MOV     DI,[FCB]
        AND     BYTE PTR ES:[DI+DEVID],0FFH-40H ;Mark as no more data available
SETFCBJ:
        JMP     SETFCB
...
;       LSTCLUS, CLUSPOS fields in FCB set
        ...
        JS      READDEV
        MOV     AX,WORD PTR ES:[DI+FILSIZ]
        MOV     BX,WORD PTR ES:[DI+FILSIZ+2]
...
;       LSTCLUS, CLUSPOS fields in FCB set
        ...
        DIV     WORD PTR [BP+SECSIZ]     ;AX=last sector accessed
        MOV     CL,[BP+CLUSSHFT]
        SHR     AX,CL           ;Last cluster to be accessed
        PUSH    AX
        MOV     AX,WORD PTR ES:[DI+FILSIZ]
        MOV     DX,WORD PTR ES:[DI+FILSIZ+2]
        DIV     WORD PTR [BP+SECSIZ]
        OR      DX,DX
        JZ      NORNDUP
...
FINWRT:
        ...
        MOV     CX,DX
        MOV     DI,[FCB]
        CMP     AX,WORD PTR ES:[DI+FILSIZ]
        SBB     CX,WORD PTR ES:[DI+FILSIZ+2]
        JB      SAMSIZ
        MOV     WORD PTR ES:[DI+FILSIZ],AX
        MOV     WORD PTR ES:[DI+FILSIZ+2],DX
SAMSIZ:
        MOV     CX,[RECCNT]
        JMP     SETCLUS
...
UPDATE:
        ...
        MOV     WORD PTR ES:[DI+FILSIZ+2],AX
        XOR     CX,CX
        JMP     ADDREC
...
;       [READOP] = 0 for read, 1 for write
        ...
        PUSH    DX
        MUL     WORD PTR [BP+SECSIZ]     ;Number of bytes transferred
        POP     DX
...
FINCLUS:
        ...
        PUSH    DX
        MOV     AX,CX
        MUL     WORD PTR [BP+SECSIZ]     ;Number of sectors times sector size
        MOV     SI,[NEXTADD]
...
HAVDIF:
        ...
        MOV     DS:[6],AX
        MOV     DS:[8],BX
        MOV     WORD PTR DS:[0],20CDH    ;"INT INTTAB"
        MOV     BYTE PTR DS:[5],LONGCALL
        RET
...
GETFATSIZ:
        ...
        DEC     WORD PTR ES:[BP+DSKSIZ] ;BP.DSKSIZ Damp those oscillations
        JMP     FNDFATSIZ ;Try again
...
HAVFATSIZ:
		STOSB                   ;FATSIZ
        MUL     BYTE PTR ES:[BP+FATCNT]  ;Space occupied by all FATs
        ADD     AX,ES:[BP+FIRFAT]
        STOSW                   ;FIRDIR
        ADD     AX,ES:[BP+DIRSEC]
...
; CX has size of memory in paragraphs (reduced by DOS size if HIGHMEM)
        ...
        MOV     WORD PTR DS:[ENTRYPOINT+1],ENTRY
        MOV     WORD PTR DS:[ENTRYPOINT+3],AX
        MOV     WORD PTR DS:[0],DIVOV   ;Set default divide trap address
        MOV     DS:[2],AX
        MOV     CX,9
        REP STOSW               ;Set 5 segments (skip 2 between each)
        MOV     WORD PTR DS:[INTBASE+4],COMMAND
        MOV     WORD PTR DS:[INTBASE+12],F_IRET   ;Ctrl-C exit
        MOV     WORD PTR DS:[INTBASE+16],F_IRET   ;Fatal error exit
        MOV     AX, BIOSREAD
        STOSW
        MOV     AX,BIOSSEG
        STOSW
        STOSW                   ;Add 2 to DI
        STOSW
        MOV     WORD PTR DS:[INTBASE+18H],BIOSWRITE
...
FIGFATSIZ:
        MUL     BYTE PTR ES:[BP+FATCNT]  ;.FATCNT
        ADD     AX,ES:[BP+FIRFAT]		;.FIRFAT
        ADD     AX,ES:[BP+DIRSEC]		;.DIRSEC/FIRREC
FIGMAX:
;AX has equivalent of FIRREC
        SUB     AX,ES:[BP+DSKSIZ]		;.DSKSIZ/MAXCLUS
        NEG     AX
        MOV     CL,ES:[BP+CLUSSHFT]		;.CLUSSHFT
        SHR     AX,CL
        INC     AX
        MOV     CX,AX           ;MAXCLUS
        INC     AX
...
FATSIZTAB	DW	16 DUP (0)
DRVCNT  DB      0

	; overlap with init code
	ORG	INITCODE
